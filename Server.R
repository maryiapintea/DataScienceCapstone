...
quadgram <- readRDS("A:\\home\\Documents\\quadgram.RData");
trigram <- readRDS("A:\\home\\Documents\\trigram.RData");
bigram <- readRDS("A:\\home\\Documents\\bigram.RData");
mesg <<- ""
...
Predict <- function(x) {
  ...
  if (length(xs)>= 3) {
    ...
    else {mesg <<- "Base N-gram for prediction: 4"; head(quadgram[quadgram$unigram == xs[1] & quadgram$bigram == xs[2] & quadgram$trigram == xs[3], 4],1)}
  }
  else if (length(xs) == 2){
    xs <- tail(xs,2)
    ...
    else {mesg<<- "Base N-gram for prediction: 3"; head(trigram[trigram$unigram == xs[1] & trigram$bigram == xs[2], 3],1)}
  }
  else if (length(xs) == 1){
    xs <- tail(xs,1)
    ...
    else {mesg <<- "Base N-gram for prediction: 2"; head(bigram[bigram$unigram == xs[1],2],1)}
  }
}
